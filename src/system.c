...
{
    /*
     * Do not call rv->io.mem_trans() because the basic block might be
...
{
    uint32_t addr = rv->io.mem_trans(rv, vaddr, R);
...
{
    uint32_t addr = rv->io.mem_trans(rv, vaddr, W);
...
/*
 * TODO: TLB could be introduced here to
 * cache the gVA to gPA tranlation. Especially
 * executing the SDL-oriented application in
 * guest OS. TLB should also applied to above
 * MMU IO handlers.
 */
static inline void mmu_memory_read(riscv_t *rv,
                                   uint8_t *dst,
                                   uint32_t addr,
                                   uint32_t size)
{
...
static inline void mmu_memory_write(riscv_t *rv,
                                    uint32_t addr,
                                    uint8_t *src,
                                    uint32_t size)
{
...
static inline void mmu_memory_fill(riscv_t *rv,
                                   uint32_t addr,
                                   uint32_t size,
                                   uint8_t val)
{
...
riscv_io_t mmu_io = {
    ...
    .mem_read = mmu_memory_read,
    .mem_write = mmu_memory_write,
    .mem_fill = mmu_memory_fill,

    /* VA2PA handler */
    .mem_trans = mmu_translate,
