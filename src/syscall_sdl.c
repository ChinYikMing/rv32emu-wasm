...
static void play_sfx(riscv_t *rv)
{
    ...
    while (remain_size > 0) {
        uint32_t sfx_data_addr =
            rv->io.mem_translate(rv, sfx_data_vaddr + curr_offset, R);
        offset = get_offset_by_addr(sfx_data_addr);
        curr_page_data_size = RV_PG_SIZE - offset;
        curr_data_size = remain_size <= curr_page_data_size
                             ? remain_size
                             : curr_page_data_size;
        memory_read(attr->mem, sfx_data_ptr + curr_offset, sfx_data_addr,
                    sizeof(uint8_t) * curr_data_size);
        remain_size -= curr_data_size;
        curr_offset += curr_data_size;
    }
...
static void play_music(riscv_t *rv)
{
    ...
    while (remain_size > 0) {
        uint32_t music_data_addr =
            rv->io.mem_translate(rv, music_data_vaddr + curr_offset, R);
        offset = get_offset_by_addr(music_data_addr);
        curr_page_data_size = RV_PG_SIZE - offset;
        curr_data_size = remain_size <= curr_page_data_size
                             ? remain_size
                             : curr_page_data_size;
        memory_read(attr->mem, music_data_ptr + curr_offset, music_data_addr,
                    sizeof(uint8_t) * curr_data_size);
        remain_size -= curr_data_size;
        curr_offset += curr_data_size;
    }
